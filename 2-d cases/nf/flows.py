...
class RealNVP(nn.Module):
    ...
    def __init__(self, dim, hidden_dim = 16, base_network=FCNN):
        ...
        self.t2 = base_network(dim // 2, dim // 2, hidden_dim)
        self.s2 = base_network(dim // 2, dim // 2, hidden_dim)
        self.t3 = base_network(dim // 2, dim // 2, hidden_dim)
        self.s3 = base_network(dim // 2, dim // 2, hidden_dim)
        self.t4 = base_network(dim // 2, dim // 2, hidden_dim)
        self.s4 = base_network(dim // 2, dim // 2, hidden_dim)
        self.t5 = base_network(dim // 2, dim // 2, hidden_dim)
        self.s5 = base_network(dim // 2, dim // 2, hidden_dim)
        self.t6 = base_network(dim // 2, dim // 2, hidden_dim)
        self.s6 = base_network(dim // 2, dim // 2, hidden_dim)
    ...
    def forward(self, x):
        ...
        s1_transformed = self.s1(lower)
        upper = t1_transformed + upper * torch.exp(s1_transformed)
        # upper = 1 / (1+torch.exp(-a*upper)) - 0.5
        upper = upper / a
        upper1 = upper.squeeze()
    
        t2_transformed = self.t2(upper)
        s2_transformed = self.s2(upper)
        lower = t2_transformed + lower * torch.exp(s2_transformed)
        # lower = 1 / (1+torch.exp(-a*lower)) - 0.5
        lower = lower / a
        lower1 = lower.squeeze()
        
        t3_transformed = self.t3(lower)
        s3_transformed = self.s3(lower)
        upper = t3_transformed + upper * torch.exp(s3_transformed)
        # upper = 1 / (1+torch.exp(-a*upper)) - 0.5
        upper  = upper / a
        upper2 = upper.squeeze()
        t4_transformed = self.t4(upper)
        s4_transformed = self.s4(upper)
        lower = t4_transformed + lower * torch.exp(s4_transformed)
        # lower = 1 / (1+torch.exp(-a*lower)) - 0.5
        lower = lower / a
        lower2 = lower.squeeze()
        
        t5_transformed = self.t5(lower)
        s5_transformed = self.s5(lower)
        upper = t5_transformed + upper * torch.exp(s5_transformed)
        # upper = 1 / (1+torch.exp(-a*upper)) - 0.5
        upper = upper / a
        upper3 = upper.squeeze()
        t6_transformed = self.t6(upper)
        s6_transformed = self.s6(upper)
        lower = t6_transformed + lower * torch.exp(s6_transformed)
        # lower = 1 / (1+torch.exp(-a*lower)) - 0.5
        lower = lower / a
        lower3 = lower.squeeze()
        
        z = torch.cat([lower, upper], dim=1)
        log_det = torch.sum(s1_transformed, dim=1) + torch.log(1/a) + \
                  torch.sum(s2_transformed, dim=1) + torch.log(1/a) + \
                  torch.sum(s3_transformed, dim=1) + torch.log(1/a) + \
                  torch.sum(s4_transformed, dim=1) + torch.log(1/a) + \
                  torch.sum(s5_transformed, dim=1) + torch.log(1/a) + \
                  torch.sum(s6_transformed, dim=1) + torch.log(1/a)
        
        # ## sigmoid function
        # log_det = torch.sum(s1_transformed, dim=1) + torch.log(torch.abs(-a*(upper1+0.5)*(1.5-upper1))) + \
        #           torch.sum(s2_transformed, dim=1) + torch.log(torch.abs(-a*(lower1+0.5)*(1.5-lower1))) + \
        #           torch.sum(s3_transformed, dim=1) + torch.log(torch.abs(-a*(upper2+0.5)*(1.5-upper2))) + \
        #           torch.sum(s4_transformed, dim=1) + torch.log(torch.abs(-a*(lower2+0.5)*(1.5-lower2))) + \
        #           torch.sum(s5_transformed, dim=1) + torch.log(torch.abs(-a*(upper3+0.5)*(1.5-upper3))) + \
        #           torch.sum(s6_transformed, dim=1) + torch.log(torch.abs(-a*(lower3+0.5)*(1.5-lower3)))
        # ##original
        # log_det = torch.sum(s1_transformed, dim=1) + \
        #           torch.sum(s2_transformed, dim=1) + \
        #           torch.sum(s3_transformed, dim=1) + \
        #           torch.sum(s4_transformed, dim=1) + \
        #           torch.sum(s5_transformed, dim=1) + \
        #           torch.sum(s6_transformed, dim=1)
        return z, log_det
    ...
    def inverse(self, z):
        ...
        t6_transformed = self.t6(upper)
        s6_transformed = self.s6(upper)
        # lower = -torch.log(1/(lower+0.5)-1) / a
        lower = a * lower
        lower = (lower - t6_transformed) * torch.exp(-s6_transformed)
        t5_transformed = self.t5(lower)
        s5_transformed = self.s5(lower)
        # upper = -torch.log(1/(upper+0.5)-1) / a
        upper = a * upper
        upper = (upper - t5_transformed) * torch.exp(-s5_transformed)
        
        t4_transformed = self.t4(upper)
        s4_transformed = self.s4(upper)
        # lower = -torch.log(1/(lower+0.5)-1) / a
        lower = a * lower
        lower = (lower - t4_transformed) * torch.exp(-s4_transformed)
        t3_transformed = self.t3(lower)
        s3_transformed = self.s3(lower)
        # upper = -torch.log(1/(upper+0.5)-1) / a
        upper = a* upper
        upper = (upper - t3_transformed) * torch.exp(-s3_transformed)
